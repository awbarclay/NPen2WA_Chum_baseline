...
load("V:\\Analysis\\5_Coastwide\\Chum\\SPen2WA_Chum_baseline\\NPen2WA_Chum_baseline_raw.Rdata")
...
The is the collection information for all of the Chum salmon baseline collections we have in LOKI.
Some of the location names and lat/longs have been corrected and collection have been ordered geographically.
```{r collection information, echo=FALSE}
...
sillyvec578 <- collection_info$Silly_Code

#LOKI2R.GCL(sillyvec=c("CHJOS92UW","CMZAC09"),username="awbarclay",password)#Pull Data from LOKI
...
tmp <- gsub(x = ls(pattern = ".gcl") [-c(1,2,385)], pattern = ".gcl", replacement = "")


collections383 <-  collection_info %>% 
...
write_csv(collections383, "output/collections383.csv")
...
samp_size_by_loc <- SampSizeByLocus.GCL(sillyvec = collections383$Silly_Code, loci = loci) %>% 
...
```
The dark cell in the heatmap are where collections are missing data at some loci.
```{r heatmap of sample sizes by locus, echo=FALSE}
...
```
Here I'm creating a new sillyvec and collections objects that only contain the sillys for collection that have all 96 of the WASSIP loci. 
```{r drop collections}
...
Here I'm dumping out the data - as a dataframe - for the 338 collections that have data at all 96 SNPs so failure rates can be calcuated on the final baseline pops after quality control measures have been take.
...
unlist(removedDups[!removedDups == "Nothing Removed"]) %>% 
  as_tibble(rownames = "silly") #Gets the individuals removed from workspace
...
SampSize(sillyvec = Nanaimo_Sillys)
...
```{r HWE in genepop for Nanaimo plates}
...
```{r drop collections-1, echo=FALSE}
...
pre_pooling <- readxl::read_xlsx(path="SPen2WA_Chum_baseline_summary.xlsx", sheet="Prepooling collections", skip = 1)
...
There were 8 pops out of HWE with alpha=0.05. After Bonferoni adjustment (0.05/nloci) 3 pops were out of HWE: CMSALM03S.CMSALM04S, CMMILCR94, and CMHAMM01.CMHAM01S.CMHAM03S. Drop the CMSALM04S collection, since it was out of HWE on it's own, CMMILCR94, and the CMHAMM01 collection    
...
Scanning in lower pairwise Fst matrtix produce by genepop: [NAKPen2WA_250pops_93loci.MIG]("GENEOP/NAKPen2WA_252pops_93loci.MIG")
```{r 250 pop tree, message=FALSE}

x <- scan(file = "GENEPOP/NAKPen2WA_250pops_93loci.MIG", skip = 3, nlines = ((250-1)*250/2), fill = TRUE, what = '') %>% as.numeric()

ndist <- seq(((250-1)*250/2))
...
dimnames(m) <- list(treenames250, treenames250)
...
Check pairwise Fst tree of the pooled collections. Assign Campbell Creek to the Knik Arm region, it looks more like Little Su and Ship than Turnagain Arm pops. Drop 
...
```{r make unrooted tree using regions, fig.height=15, fig.width=10}
